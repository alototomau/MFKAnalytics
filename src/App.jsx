...
const services = [
  "Analytics automation and executive dashboards",
  "Semantic models, DAX and performance tuning",
  "Microsoft Fabric migrations and architecture",
  "ETL/ELT pipelines with Python and PySpark",
  "Lakehouse and Data Warehouse foundations",
  "Corporate training in Power BI, SQL and Fabric",
];

const stages = [
  { icon: Layers3, title: "01 · Discover", text: "Business context, data sources, KPIs, pain points and automation opportunities." },
...
const recommendations = [
  ...
  { author: "Alexander Bermúdez Muñoz", role: "Data Scientist | Data Analyst | Machine Learning | Python | SQL | Azure | AWS", quote: "Mauricio is a true change agent. He is always learning, always finding ways to make projects more automated and valuable for the teams he works with." },
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 lg:px-10">
        <a href="#home" className="group flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 leading-tight">
            <p className="truncate font-black tracking-tight text-[#445065] max-sm:text-sm">MFK Analytics</p>
  ...
  return (
    <main className="min-h-screen scroll-smooth bg-[#f4f1ea] text-[#1f2937]">
      ...
      <section id="home" className="relative overflow-hidden bg-gradient-to-br from-[#f7fbff] via-[#eefbf7] to-[#dbe7f8] pt-20 lg:pt-20">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:py-20 lg:grid-cols-[1fr_0.95fr] lg:px-10 lg:py-24">
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="max-w-4xl text-4xl font-black leading-[0.95] tracking-tight text-[#445065] sm:text-5xl md:text-7xl">
            ...
            <p className="mt-7 max-w-2xl text-base leading-8 text-[#526074] sm:text-lg">
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.35 }} className="mt-9 grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:gap-4">
              <a href="#services">
                <Button className="w-full rounded-2xl bg-[#07111f] px-4 py-5 text-sm font-bold text-white hover:bg-[#172033] sm:w-auto sm:px-6 sm:py-6 sm:text-base">
                  Explore Services <ArrowRight className="ml-2" size={18} />
                </Button>
              </a>
              <a href={cvUrl} download>
                <Button variant="outline" className="w-full rounded-2xl border-[#44d6ad] bg-white/80 px-4 py-5 text-sm font-bold text-[#445065] hover:bg-white sm:w-auto sm:px-6 sm:py-6 sm:text-base">
                  <Download className="mr-2" size={18} /> Download CV
                </Button>
              </a>
              <a className="col-span-2 sm:col-span-1" href={linkedInUrl} target="_blank" rel="noreferrer">
                <Button variant="outline" className="w-full rounded-2xl border-slate-300 bg-white/60 px-6 py-5 text-base text-[#445065] hover:bg-white sm:w-auto sm:py-6">
                  <span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded bg-[#44d6ad] text-xs font-black text-white">in</span> LinkedIn
                </Button>
              </a>
            </motion.div>
          </motion.div>
      ...
      <section className="relative overflow-hidden px-6 py-24 lg:px-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(68,214,173,0.12),transparent_28%),radial-gradient(circle_at_80%_10%,rgba(191,210,243,0.22),transparent_34%)]" />
        <div className="relative mx-auto max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }}>
              <p className="text-sm font-black uppercase tracking-[0.34em] text-[#23b895]">Trusted experience</p>
              <h2 className="mt-4 text-4xl font-black tracking-tight text-[#445065] md:text-5xl">Companies and projects served</h2>
              <p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-[#526074]">
                Experience supporting analytics, reporting and data initiatives across consulting, retail, real estate, flooring, health, entertainment, technology and enterprise services.
              </p>
            </motion.div>
          </div>
        </div>

        <div className="relative mt-14 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)] [-webkit-mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
          <motion.div animate={{ x: [0, -1520] }} transition={{ duration: 46, repeat: Infinity, ease: "linear" }} className="flex w-max gap-6">
            {companyLogoLoop.map((company, index) => (
              <motion.div
                key={`${company.name}-${index}`}
                whileHover={{ scale: 1.08, y: -8 }}
                transition={{ type: "spring", stiffness: 260, damping: 18 }}
                className="group relative flex h-28 w-64 items-center justify-center overflow-hidden rounded-[2rem] border border-white/80 bg-white/85 px-7 py-5 shadow-sm backdrop-blur transition-all hover:border-[#44d6ad]/50 hover:bg-white hover:shadow-2xl"
              >
                <div className="absolute inset-x-8 bottom-0 h-1 rounded-full bg-gradient-to-r from-transparent via-[#44d6ad]/70 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                {company.logo ? (
                  <img
                    src={company.logo}
                    alt={`${company.name} logo`}
                    className="relative max-h-16 max-w-[190px] object-contain opacity-90 saturate-90 transition-all duration-300 group-hover:scale-105 group-hover:opacity-100 group-hover:saturate-150"
                  />
                ) : (
                  <span className="relative text-xl font-black tracking-wide text-[#445065] opacity-85 transition-all group-hover:scale-105 group-hover:text-[#23b895] group-hover:opacity-100">
                    {company.logoText}
                  </span>
                )}
              </motion.div>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.65 }} id="services" className="scroll-mt-32 mx-auto max-w-7xl px-6 py-16 lg:px-10">
      ...
      <section className="bg-[#07111f] px-6 py-16 text-white lg:px-10">
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.65 }} className="mx-auto max-w-7xl">
          <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.3em] text-[#44d6ad]">Services</p><h2 className="mt-3 text-4xl font-black md:text-5xl">Analytics automation for business teams</h2></div><a href={linkedInUrl} target="_blank" rel="noreferrer" className="text-[#a9f5e2] hover:text-white">Connect on LinkedIn →</a></div>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.65 }} className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
        <div className="mb-10"><p className="text-sm font-bold uppercase tracking-[0.3em] text-[#44bfa0]">Portfolio</p><h2 className="mt-3 text-4xl font-black text-[#445065] md:text-5xl">Featured work</h2><p className="mt-4 max-w-2xl text-[#526074]">In this section, you can explore selected analytics, automation and business intelligence projects designed to turn data into practical decisions.</p></div>
      ...
      <motion.section id="experience" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.65 }} className="scroll-mt-32 mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-2 lg:px-10">
      ...
      <section id="recommendations" className="scroll-mt-32 relative overflow-hidden bg-gradient-to-br from-[#e8f7f4] via-[#f7fbff] to-[#dfe9f8] px-6 py-20 lg:px-10">
      ...
      <motion.section id="contact" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.65 }} className="scroll-mt-32 mx-auto max-w-7xl px-6 py-20 lg:px-10">
        <Card className="overflow-hidden rounded-[2rem] border-white bg-[#07111f] text-white shadow-xl md:rounded-[2.5rem]">
          <CardContent className="grid gap-0 p-0 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative overflow-hidden bg-gradient-to-br from-[#07111f] via-[#0b1b2d] to-[#10243a] p-8 md:p-10">
              ...
              <div className="relative">
                ...
                <p className="mt-5 max-w-xl text-slate-200">Tell me what you need: dashboards, automation, Microsoft Fabric, semantic models, data pipelines, training or analytics advisory.</p>
                <div className="mt-8 grid gap-3 text-sm text-slate-100">
                  {["Analytics automation, dashboards and executive reporting", "Microsoft Fabric architecture and implementation", "Corporate training for analytics teams", "Data engineering, semantic models and automation"].map((item) => (
                    <div key={item} className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-[#44d6ad]" size={18} /><span>{item}</span></div>
                  ))}
                </div>
                <div className="mt-8 flex flex-wrap gap-3">
                  <a href={linkedInUrl} target="_blank" rel="noreferrer"><Button className="rounded-2xl bg-[#44d6ad] px-5 py-5 font-bold text-slate-950 hover:bg-[#7ee7cb]">Connect on LinkedIn</Button></a>
                  <a href={cvUrl} download><Button variant="outline" className="rounded-2xl border-white/20 bg-white/5 px-5 py-5 text-white hover:bg-white/10"><Download className="mr-2" size={18} /> Download CV</Button></a>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 text-[#1f2937] md:p-10">
              <form action="https://formspree.io/f/meedrplp" method="POST" className="grid gap-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div><label className="mb-2 block text-sm font-bold text-[#445065]">Work email</label><input name="email" type="email" required className="w-full rounded-2xl border border-slate-200 bg-[#f7fbff] px-4 py-3 outline-none transition focus:border-[#44d6ad] focus:ring-4 focus:ring-[#44d6ad]/15" placeholder="name@company.com" /></div>
                  <div><label className="mb-2 block text-sm font-bold text-[#445065]">Company</label><input name="company" className="w-full rounded-2xl border border-slate-200 bg-[#f7fbff] px-4 py-3 outline-none transition focus:border-[#44d6ad] focus:ring-4 focus:ring-[#44d6ad]/15" placeholder="Company name" /></div>
                </div>
                <div>
                ...
                <div><label className="mb-2 block text-sm font-bold text-[#445065]">Message</label><textarea name="message" required rows="5" className="w-full resize-none rounded-2xl border border-slate-200 bg-[#f7fbff] px-4 py-3 outline-none transition focus:border-[#44d6ad] focus:ring-4 focus:ring-[#44d6ad]/15" placeholder="Tell me about your project, timeline and goals." /></div>
                <Button type="submit" className="mt-2 rounded-2xl bg-[#07111f] px-6 py-5 text-base font-bold text-white hover:bg-[#172033]">Request Consultation <ArrowRight className="ml-2" size={18} /></Button>
              </form>
            </div>
          </CardContent>
        </Card>
      ...
      <footer className="border-t border-white px-6 py-8 text-center text-sm text-[#6b7280]">© {new Date().getFullYear()} MFK Analytics · Mauricio Montoya Huertas. Analytics Automation, Business Intelligence and Data Engineering.</footer>
    </main>
  );
}
